...
function addGamesToPage(games) {
  ...
  for (var i = 0; i < games.length; i++) {
    ...
    gameDiv.innerHTML = `
    <h4>${item.name}</h4>
        <img class="game-img" src="${item.img}" alt="Title Thumbnail" />
        <p>${item.description}</p>
        <progress max="${item.goal}" value="${item.pledged}"></progress>
...
const totalContributions = GAMES_JSON.reduce((sum, game) => {
  return sum + game.backers; //Each return becomes the new sum parameter for every "game"
}, 0);
...
const totalRaised = GAMES_JSON.reduce((sum, game) => {
  return sum + game.pledged; //Each return becomes the new sum parameter for every "game"
}, 0);
...
} game(s) remain unfunded. We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
var descDiv = document.createElement("paragraph");
